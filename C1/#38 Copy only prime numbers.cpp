...
enum enPrinNotPrime { Prime = 1, NotPrime = 2 };

enPrinNotPrime CheckPrime(int Number)
{
    ...
    {
        if (Number % Counter == 0)
            return enPrinNotPrime::NotPrime;
    }
...
{
    for (int i = 0; i < arrLength; i++)
        if (CheckPrime(arrSource[i]) == enPrinNotPrime::Prime)
        {
